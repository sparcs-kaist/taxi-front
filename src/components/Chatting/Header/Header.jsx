...
const BtnCancel = (props) => {
  ...
  const style = useSpring({
    ...
    color: 'white', fontSize: '15px', fontWeight: 300,
...
const Info = (props) => {
  const style1 = {
    fontSize: '11px', fontWeight: 300, color: '#888888',
    textAlign: props.align
  }
  const style2 = {
    fontSize: '13px', fontWeight: 300, color: 'black',
    textAlign: props.align
  }
  return (
    <div>
...
const User = (props) => {
  const style = {
    fontSize: '13px', fontWeight: 300, color: '#888888',
    height: '28px', lineHeight: '28px', position: 'relative',
    paddingLeft: '40px', paddingRight: '7px'
  }
  const styleImg = {
  ...
  const styleBg = {
    ...
    top: '1px', bottom: '1px',
    left: '33px', right: '0px',
    background: '#EEEEEE', borderRadius: '5px'
  }
  const styleNickname = {
    position: 'absolute',
    top: '0px', right: '7px',
  ...
  return (
    <div style={ style }>
      <img style={ styleImg } src={ `${ backServer }/static/profile-images/${ props.id }` } alt=""/>
      <div style={ styleBg }/>
...
const transDate = (x) => {
  ...
  return `${ year }년 ${ month }월 ${ day }일(${ week }) ${ hour>=12 ? '오후' : '오전' } ${ hour>12 ? hour-12 : hour }시 ${ min }분`;
}
const HeaderBottom = (props) => {
  ...
  const styleLay1 = {
    ...
    gap: '5px 15px', justifyContent: 'space-between'
  }
  ...
  return (
    <div className="chatting-header-bottom">
      ...
      <div style={ styleLay1 }>
        <Info title="출발 시각 & 날짜" align="left">{ madeat }</Info>
        <div style={ styleLay1Right }>
          <Info title="개설자" align="right">어궁동 패티</Info>
          <Info title="결제자" align="right">결제 미완료</Info>
          <Info title="정산 여부" align="right">No</Info>
        </div>
      </div>
      <div style={{ height: '20px' }}/>
      <div style={ styleLay2 }>
        { part.map((item, index) => <User key={ index } id={ item.id } nickname={ item.nickname }/>) }
      </div>
      <div style={{ height: '20px' }}/>
    </div>
  )
...
const Header = (props) => {
  ...
  const [btmSize, setBtmSize] = useState(114);
	const style = useSpring({
		position: 'fixed', top: '0px', left: '0px',
		width: '100%', height: (isOpen ? `${ 80 + btmSize }px` : '80px'),
  ...
  const styleSubtitle = {
		position: 'absolute', top: '45px', left: '70px', right: '70px',
		fontSize: '13px', fontWeight: 300, color: '#888888'
  }
  const styleBorder = {
    height: "1px", marginLeft: "15px", marginRight: "15px",
  ...
  useEffect(() => {
	...
	return (
        ...
        <div style={{ position: "relative", height: "80px" }}>
          ...
          <div style={ styleSubtitle }>{ props.info ? `${ props.info.from } → ${ props.info.to }` : "" }</div>
          <BtnBack/>
