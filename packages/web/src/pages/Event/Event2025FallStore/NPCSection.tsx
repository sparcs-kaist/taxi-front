...
import { randomEvent2025FallStoreSpeechGenerator } from "@/tools/random";
...
const NPCSection = () => {
  ...
  const [currentText, setCurrentText] = useState<string>(
    randomEvent2025FallStoreSpeechGenerator()
  );

  useEffect(() => {
    if (text.length === currentText.length) {
      const timeout = setTimeout(() => {
        setCurrentText(randomEvent2025FallStoreSpeechGenerator());
        setText("");
      }, 2000);
