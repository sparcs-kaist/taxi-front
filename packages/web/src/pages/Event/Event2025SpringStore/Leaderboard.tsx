...
interface Event2025SpringStoreLeaderboardProps {
  itemId: string;
}

const Event2025SpringStoreLeaderboard = ({
  itemId,
}: Event2025SpringStoreLeaderboardProps) => {
  ...
  const [isDisabled, buttonText] = useMemo(
    () =>
      eventMode !== "2025spring"
        ...
        ? [true, "송편코인이 부족합니다"]
        : [false, "응모권 구매하기"],
    [eventMode, event2025SpringInfo, item, purchaseAmount]
  );
  ...
  const onClickOk = useCallback(async () => {
    ...
    await axios({
      url: `/events/2025spring/items/purchase/${item?._id}`,
      method: "post",
      data: {
  ...
  }, [item?._id, event2025SpringInfo, purchaseAmount]);
...
export default Event2025SpringStoreLeaderboard;
