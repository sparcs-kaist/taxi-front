...
import { randomEvent2024FallStoreSpeechGenerator } from "@/tools/random";
...
const NPCSection = () => {
  ...
  const [currentText, setCurrentText] = useState<string>(
    randomEvent2024FallStoreSpeechGenerator()
  );

  useEffect(() => {
    if (text.length === currentText.length) {
      const timeout = setTimeout(() => {
        setCurrentText(randomEvent2024FallStoreSpeechGenerator());
        setText("");
      }, 2000);
